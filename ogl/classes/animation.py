...
class Animation:
    ...
    def include(self) -> pygame.Surface:
       image = pygame.image.load(self.self_paths[
           trunc(self.count) % self.lenght()
        ])
...
class SymmetricalAnimation(Animation):
    ...
    def __str__(self) -> str:
        return f"<SymmetricalAnimation lenght:{self.lenght( * 2)} time:{self.time} frame>"
class AnimatedObject(window.WindowObject):
    def __init__(self, animation:Animation|None, x: int = 0, y: int = 0,\
                 width:int = 50, height:int = 50) -> None:
...
class MultiAnimationsObject(AnimatedObject):
    def __init__(self, animations: list[Animation] = [], x: int = 0,\
