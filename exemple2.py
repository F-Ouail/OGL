...
class Player(ogl.basic.Square):
    def __init__(self, pos: tuple[int, int], speed: int) -> None:
        super().__init__(
            ogl.color.Color(252, 226, 196), pos[0], pos[1], width=50, height=50
        )
    ...
    def move(
        self,
        vertically: tuple[bool, bool],
        horizontally: tuple[bool, bool],
        delta: float,
    ):
        ...
        y = (
            ...
            else False if horizontally[0] else True
        )
...
def main() -> int:
    # Create Objects
    ## Create the Window
    window = ogl.window.Window(title="exemple 2", auto_draw=True, resizability=False)
    ## Create Player
    ...
    button_1 = ogl.ui.Button(text="Play", y=120)
    button_2 = ogl.ui.Button(text="Exit", y=240)
    ## Create the Scene (a Container)
    scene_1 = ogl.scene.Scene(window, name="scene 1", background=(28, 120, 71))
    scene_2 = ogl.scene.Scene(window, name="scene 2", background=(0x56, 0x7D, 0x46))
    # Link Objects
    ...
    # Setup handles
    window.mouse_handler = lambda *args: on_click(
        window, scene_1, scene_2, button_1, button_2, *args
    )
    window.keyboard_handler = lambda *args: keyboard_handler(
        window, scene_1, scene_2, player, *args
    )
    window.on_key_down = lambda *args: keydown(window, scene_1, scene_2, player, *args)
    scene_1.set_should_draw(True)
    # the Window Loop
