...
class Scene(window.WindowObject):
    def __init__(self, _window:window.Window, name:str,\
        ...
        if background is None:
            ...
            raise TypeError(f"The background type is one of Color, None,"+
                f"tuple[int, int, int].The Entred type is {type(background)}")
        _window.append(self)
        self.save = ()
    ...
    def append_object(self, obj:window.WindowObject,
                      xc:bool = False, yc:bool = False) -> None:
        obj._mouse = window.mouse.pos
        self.linked_objects.append((obj, xc, yc))
        self.reset()
    def reset(self) -> None:
