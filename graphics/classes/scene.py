...
class Scene(window.WindowObject):
    def __init__(self, _window:window.Window, name:str,\
        ...
        if background is None:
            ...
            if not (type(background[0]) == type(background[1]) == type(background[2]) == int):
                raise TypeError(f"The background Color is not tuple[{type(background[0])}, {type(background[1])}, {type(background[2])}]")
            self.background = color.Color(background[0],
                                          background[1],
                                          background[2])
        elif type(background) is color.Color:
            self.background = background
        else:
            raise TypeError("The background type is one of Color,"
    ...
    def input_to_update(self, *args) -> None:
        self.save = args
        return
    def append_most_priority_object(self,
                                    object:window.WindowObject) -> None:
    ...
    def append_object(self, obj:window.WindowObject,
                      centred_horizontally:bool = False,
                      centred_vertically:bool = False) -> None:
        obj._mouse = window.mouse.pos
        self.linked_objects.append((obj, centred_horizontally,                
                                    centred_vertically))
        self.reset()
    def reset(self) -> None:
